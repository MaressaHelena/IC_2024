...
void print_help(std::vector<std::string> const& required_parameters)
{
   ...
   oss << "Usage: analysis-Dijets-HepMC ";
...
int main(int argc, char** argv) { 
   ...
   */
   ...
   double jet_eta[JETMAX];
   double jet_rap[JETMAX];
   double jet_phi[JETMAX];
   ...
   */
   ...
   T->Branch("jet_eta", &jet_eta,"jet_eta[n_jet]/D");
   T->Branch("jet_rap", &jet_rap,"jet_rap[n_jet]/D");
   T->Branch("jet_phi", &jet_phi,"jet_phi[n_jet]/D");
   ...
   */
    ...
    */
    ...
    /*
	 if ( isfinal_(*p) ){
	    ...
	    if( charge != 0. && fabs( (*p)->momentum().eta() ) <= 2.5 && (*p)->momentum().perp() >= 1.0) {
	       chg_id[n_chg] = pdg_id;
	       chg_ch[n_chg] = charge;
    ...
    */
	 ...
	 jet_eta[i] = sortedJets[i].eta();
	 jet_rap[i] = sortedJets[i].rapidity();
	 jet_phi[i] = sortedJets[i].phi();
